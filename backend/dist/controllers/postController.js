...
export const generatePost = async (req, res) => {
    try {
        ...
        if (!prompt || typeof prompt !== "string" || prompt.trim() === "") {
            res.status(400).json({ message: "Please provide a prompt for post generation" });
            return;
        }
...
Return ONLY a valid JSON object in this exact schema without any markdown formatting or code blocks:
{
  ...
  "imagePrompt": "a descriptive, detailed prompt for generating an illustrative image"
        ...
        try {
            ...
            console.warn("Could not parse JSON from Gemini, using raw text:", parseErr);
        ...
        if (generateImage && imagePrompt) {
            try {
                ...
                console.warn("Imagen generation failed or not available, proceeding without image:", imageErr?.message || imageErr);
            }
        }
...
export const schedulePosts = async (req, res) => {
    try {
        const { content, platforms, scheduledFor, status = "scheduled", mediaUrl: bodyMediaUrl, mediaType: bodyMediaType } = req.body;
