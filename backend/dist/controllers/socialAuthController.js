...
export const gerOrCreateZernioProfile = async (user) => {
    try {
        if (user.zernioProfileId && typeof user.zernioProfileId === "string" && user.zernioProfileId.trim() !== "") {
        ...
        const created = createResult.data?.profile || createResult.data?.data || createResult.data;
...
export const generateOAuthURL = async (req, res) => {
    try {
        ...
        if (!authUrl) {
            throw new Error(`Failed to generate OAuth URL. Response: ${JSON.stringify(data)}`);
        }
...
// GET /api/oauth/sync
export const syncConnectedAccounts = async (req, res) => {
    try {
        const profileId = await gerOrCreateZernioProfile(req.user);
        ...
        const data = result.data;
        const zernioAccounts = data?.accounts || (Array.isArray(data) ? data : data?.data || []);
        ...
        for (const account of zernioAccounts) {
            ...
            const normalizedPlatform = supportedPlatform.find((p) => rawPlatform.includes(p)) || rawPlatform;
            const accountRecord = await Account.findOneAndUpdate({ zernioAccountId: zid }, {
                user: req.user._id,
                platform: normalizedPlatform,
                handle: account.username || account.name || account.handle || "Connected Account",
                zernioAccountId: zid,
                status: "connected",
        ...
        res.status(500).json({ message: err?.message || "Failed to sync accounts" });
    }
};
